...
_replace_nums = lambda string: re.sub('\d+', '{}', string)
_replace_content = lambda string: re.sub('|'.join(_LABELFORMATS), '', string)
...
_VARIABLES['variables'] = _VARIABLES['label'].apply(lambda x: _replace_content(x))
_VARIABLES = _VARIABLES.set_index('variables', drop=True)
...
class USCensus_Variable(USCensus_VariableSgmts):
    ...
    @property
    def concept(self):     
        variable = _replace_nums(_uncomma_nums(_unspace_nums(self.label[-1])))       
        label = _VARIABLES.loc[variable, 'label'] 
        concept = _VARIABLES.loc[variable, 'concept']
        
        unformated = parse(label, _uncomma_nums(_unspace_nums(self.label[-1])))
    ...
    def __eq__(self, other):
        if isinstance(other, type(self)): return super().__eq__(other)
            ...
            if other[-1] == _LABELALL: return other[:-1] == self.label[:-1]
            else: return other == self.label
